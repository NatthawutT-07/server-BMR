...
exports.getAnalysisFilters = async (req, res) => {
    try {
        const [branchCodes, brands, reasons] = await Promise.all([
...
exports.getStoreAnalysis = async (req, res) => {
    try {
        ...
        // ====================================================
        let storeWithdrawReasonFilter = "AND \"reason\" != 'เบิกเพื่อขาย'";
        if (reasons?.length > 0) {
            storeWithdrawReasonFilter = `AND "reason" IN (${reasons.map(r => `'${r.replace(/'/g, "''")}'`).join(",")})`;
        }
...
exports.getBrandAnalysis = async (req, res) => {
    try {
        ...
        let brandWithdrawReasonFilter = "AND \"reason\" != 'เบิกเพื่อขาย'";
        if (reasons?.length > 0) {
            brandWithdrawReasonFilter = `AND "reason" IN (${reasons.map(r => `'${r.replace(/'/g, "''")}'`).join(",")})`;
        }
...
// ยอดขาย + ตัดจ่ายรวมรายสาขา แยกรายเดือน (สูงสุด 5 เดือน)
// ============================================================
exports.getStoreSummary = async (req, res) => {
    try {
        ...
        // ====================================================
        let summaryWithdrawReasonFilter = "AND \"reason\" != 'เบิกเพื่อขาย'";
        if (reasons?.length > 0) {
            summaryWithdrawReasonFilter = `AND "reason" IN (${reasons.map(r => `'${r.replace(/'/g, "''")}'`).join(",")})`;
        }
